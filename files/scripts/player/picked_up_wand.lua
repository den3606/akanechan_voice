...
function item_pickup(item_entity_id, pickupper_entity_id, item_name)
  ...
  local is_swapped_wand = getInternalVariableValue(player_entity_id, GLOBAL_PREFIXES:IS_THROWED_WAND(), "value_bool") or false
  if is_swapped_wand then return end

  if player_entity_id == pickupper_entity_id then
    WaitFrame:tryCall(player_entity_id, GLOBAL_PREFIXES:PICKED_ITEM(), function()
